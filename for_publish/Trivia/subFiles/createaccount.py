...
class CreateAcount(QDialog):
	...
	def send_to_server(self):
		"""
		this finction send all the necessary data about the new user to the server.
		
		
		:return
			if the server send error: return the type of error ( create account failed/email exist/username exists)
		...
		if global_vers.server_connection == None:
			...
			return
		new_user = User(
			0, self.email.text(), self.username.text(), self.password.text(),
			datetime.now().strftime("%d/%m/%Y %H:%M:%S"), 0
		)  # create user object
		cmd = client.create_account(global_vers.server_connection, new_user)  # send to the server for answer
